...
from keras.layers import Input, Dense, Reshape, Flatten, Dropout, Concatenate, Conv2DTranspose, concatenate
...
class Pix2Pix():
    ...
    def build_generator(self):
        ...
        # Image input
        e0 = Input(shape=(128, 128, 3))

        # Downsampling
        e1 = conv2d(e0, 64, bn=False)
        e2 = conv2d(e1, 64*2)
        e3 = conv2d(e2, 64*4)
        e4 = conv2d(e3, 64*8)
        e5 = conv2d(e4, 64*8)
        ...
        GRU_input = concatenate([d7, e7])

        # Upsampling
        u1 = deconv2d(d7, d6, 64*8)
        u2 = deconv2d(u1, d5, 64*8)
        u3 = deconv2d(u2, d4, 64*8)
        u4 = deconv2d(u3, d3, 64*4)
